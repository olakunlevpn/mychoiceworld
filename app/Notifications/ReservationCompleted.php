...
class ReservationCompleted extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ->subject(__('notification.reservation_completed_subject'))
            ->line(__('notification.reservation_completed_line', [
                'code' => $this->reservation->reservation_code,
                'product' => $this->reservation->product->name,
                'store' => $this->reservation->vendor->store_name,
            ]))
            ->action(__('notification.leave_review'), url('/customer/reservations/'.$this->reservation->id));
    }
    ...
    public function toArray(object $notifiable): array
    {
        return [
            ...
            'message' => __('notification.reservation_completed_line', [
                'code' => $this->reservation->reservation_code,
                'product' => $this->reservation->product->name,
                'store' => $this->reservation->vendor->store_name,
            ]),
